...
class AgentBase_ATO_ICNN:
    ...
    def explore_one_env(self, env, target_step) -> list:
        ...
        while step_i < target_step or not done:
            ...
            ten_a_m = projection_function(ten_a, ten_s, self.config)
            next_s, reward, done, _ = env.step(ten_a_m[0].numpy())  # different
    ...
    @staticmethod
    def optimizer_update_clip(optimizer, objective, model, clip_norm):
        """minimize the optimization objective via update the network parameters
    ...
    def save_or_load_agent(self, cwd, if_save):
        ...
        if if_save:
            ...
            for name, obj in name_obj_list:
                save_path = f"{cwd}/{name}.pth"
                load_torch_file(obj, save_path) if os.path.isfile(save_path) else print('Train from scratch.')
