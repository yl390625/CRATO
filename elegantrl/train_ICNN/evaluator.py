...
import torch
import numpy as np
from elegantrl.envs.ATO_env.gym_utils import projection_function
...
class Evaluator:  # [ElegantRL.2022.01.01]
    ...
    def evaluate_save_and_plot(
        ...
        if time.time() - self.eval_time < self.eval_gap:
            ...
            """evaluate first time"""
            rewards_steps_list = [
                get_episode_return_and_step(self.eval_env, act,self.config)
            ...
            if_save = r_avg < self.r_min
            if if_save:  # save checkpoint with highest episode return
                self.r_min = r_avg  # update max reward (episode return)

                act_path = (
                    f"{self.cwd}/actor_{self.total_step:08}_{self.r_min:09.3f}.pth"
                )
                torch.save(act.state_dict(), act_path)  # save policy network in *.pth

                print(
                    f"{self.agent_id:<3}{self.total_step:8.2e}{self.r_min:8.2f} |"
            ...
            if_reach_goal = bool(self.r_min > self.target_return)  # check if_reach_goal
            ...
            save_title = (
                f"step_time_maxR_{int(total_step)}_{int(train_time)}_{self.r_min:.3f}"
            )
...
def get_episode_return_and_step(env, act, config, warmup=0) -> (float, int):  # [ElegantRL.2022.01.01]
    """Usage
    eval_times = 4
    net_dim = 2 ** 7
    ...
    for episode_step in range(max_step):
        ...
        a_tensor = projection_function(act(s_tensor),s_tensor,config)
        if if_discrete:
